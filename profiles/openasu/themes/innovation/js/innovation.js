...
(function ($, Drupal) {
  ...
  $(window).scroll(function () {
    // WEBSPARK-879 - No rendering ghostSlider sticky menu in admin pages.
    if (Drupal.settings.webspark_megamenu.path_is_admin !== 1) {
      ...
      if (($('#ASUNavMenu').offset().top - ($(window).scrollTop() + navOffset)) < 1 && $('.ghostSlider').length < 1) {
        $('#ASUNavMenu').clone(true).prependTo(document.body).addClass('ghostSlider')
          .css({"position": "fixed", "z-index": "201", "width": "100%", "visibility": "visible"})
          .removeAttr('id');
  ...
  // Calculate current offset with respect to scroll position
  function offsetTop() {
