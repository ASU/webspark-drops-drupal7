                    ...
                    if (self.sorted_people[ASUPeople[field_id].dept_nid] == null) {
                        ...
                        jQuery.ajax(options).done(function (data) {
                            ...
                            for (var i = 0; i < docs.length; i++) {
                                ...
                                //find the position of the current deptid in the person's
                                // deptids array
                                var position = temp.deptids.indexOf(search);

                                if (position >= 0) {

                                    // create property for mapped manager value
                                    if (temp.hasOwnProperty('managers')) {
                                        temp.manmap = temp.managers[position];
                                    }

                                    //get the person's employee weight for this department
                                    if (!temp.hasOwnProperty('employeeWeight') || temp.employeeWeight[position] == "") {
                                        docs[i].mappedWeight = 999;
                                    } else {
                                        docs[i].mappedWeight = parseInt(temp.employeeWeight[position]);
                                    }
                                }
                            }
                            ...
                            docs.sort(function (a, b) {
                                ...
                                //todo:  remove when managers field is deprecated
